 ...
 * @param addrlen 
 */
void HTTP_server::acceptConn(SocketInfo* info, sockaddr_in address, int addrlen){
    int cfd;
 ...
 * @param query_string 
 */
void HTTP_server::execute_cgi(SocketInfo* info,  const char* path, const char* method, const char* query_string){
    char buffer[1024];
    ...
    if(std::strcmp(method, "GET") == 0)
        while((num_chars>0)&&std::strcmp("\n", buffer))
            num_chars = get_line(info, buffer, sizeof(buffer));
    else{//如果是POST请求
        num_chars = get_line(info, buffer, sizeof(buffer));
        while((num_chars>0)&&std::strcmp("\n", buffer)){
 ...
 * @param filename 
 */
void HTTP_server::server_file(SocketInfo* info, const char* filename){
 ...
 * 
 */
int HTTP_server::get_line(SocketInfo* info, char* buffer, const int size){
 ...
 * @param sock 
 */
void HTTP_server::accept_request(SocketInfo* info){
    ...
    num_chars = get_line(info, buffer, sizeof(buffer));//从客户端接收http请求
    i=0; j=0;
    ...
    if(stat(path, &st)==-1){//不存在
        ...
        if(!cgi)
            server_file(info, path);//直接提供文件内容
        else    
            execute_cgi(info, path, method, query_string);//调用CGI程序
    }
    close(sock);
}
    ...
    while(1){
        ...
        for(int i=0;i<num;i++){
            ...
            if(fd == server_sock){
                ...
                info->fd = fd;
                info->epfd = epfd;
                std::thread th(std::bind(&HTTP_server::acceptConn, &http, info, client_name, addrlen));
                th.detach();
            }
            else{
                std::thread::id threadId = std::this_thread::get_id();
                ...
                info->fd = fd;
                info->epfd = epfd;
                pool.enqueue(std::bind(&HTTP_server::accept_request, &http, info), info);
            }
        }
    }      
    close(server_sock);
