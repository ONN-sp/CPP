...
20. leveldb是怎么删除kv的=>写入一个删除标记的方式，不是原地删除，为什么这样
    * 写入性能高：删除操作和写入操作统一，只是写入一个特殊的标记，而不需要查找或覆盖元数据，这是一个顺序写过程，性能极高
    * 简化系统设计，不需要理解定位并删除磁盘上的数据，避免了复杂的IO
    * 删除标记会随着Compaction过程被清理，只有比此时最小的快照序列号小的才能删除
...
22. `LevelDB`中的`MemTable`利用`Arena`内存池进行内存管理?
