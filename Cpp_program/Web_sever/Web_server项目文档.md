...
3. 我主要就是进行了小的优化，通过swap()函数来将共享资源拷贝到局部变量，从而减小锁的范围；使用移动语义的move函数
4. 在此过程中我所遇到的问题:
   * 智能指针的胡乱使用,导致在`clear`数组时直接触发了`Channel`对象的析构函数,而此时的`Channel`对象实际可能是活跃的,导致逻辑错误
   * `std::move`的错误使用,当使用了`std::move`后还访问之前的对象,导致段错误
