...
13. 可以先声明一个线程,再初始化:
    ...
    ```
14. 线程同步=保护共享资源
15. 如果没有`thread.join()`,主线程不会等待子线程执行完毕才继续往下执行.此时可能主线程都`return`了,而子线程还在执行,此时就会报错
16. <mark>对于15,如果想主线程不等待子线程结束,但是主线程结束后子线程仍然可以在后台运行(即不报错),那么就是`thread.detach()`.此时在终端可能打印不了什么东西,因为主线程在结束时子线程还没`std::cout`</mark>
17. 多线程处理时的并行原理:
   ```C++
   #include <iostream>
   #include <thread>
...
18. 多线程的并行
   ...
   ```
19. <mark>`.join()`方法只会阻塞调用它的线程(主线程),而不会影响其它线程</mark>
20. <mark>在`C++`中,一旦线程函数执行完成并退出(一般是一个`while(1)`),线程就结束了</mark>
21. 在写多线程时,要分清哪些代码属于一个线程,哪些代码是另一个线程
22. <mark>对于主线程,当它创建了一个子线程(执行`task`的子线程),那么它会继续往下执行,而不会等待子线程的`task`执行完毕才往下走:</mark>
    ```C++
    int main(){
    std::thread t;
    t = std::thread([](){
