...
5. `SQL`标准提出了四种隔离级别来规避上述的并发事务问题,隔离级别越高,性能效率越低.串行化(串行化隔离级别通常是通过加锁来实现的)>可重复读>读已提交>读未提交
...
8. `InnoDB`的内存结构:
   * 缓冲池:缓冲池是主内存中的一个区域,里面可以缓存磁盘上经常操作的真实数据,在执行增删改查操作时,先操作缓冲池中的数据(若缓冲池每有数据,则从磁盘加载并缓存),然后再以一定频率刷新到磁盘,从而减少磁盘IO,加快处理速度.缓冲池以页为单位,底层采用链表数据结构管理页.根据状态可以分为:
      - free page:空闲页,未被使用
...
10. `InnoDB`后台线程将`InnoDB`存储引擎缓冲池刷新到磁盘文件
    ...
    * `IO thread`:在`InnoDB`存储引擎中大佬使用了异步IO,这样可以极大地提高数据库的性能
