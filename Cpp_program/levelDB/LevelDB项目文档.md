...
3. 我曾用 LevelDB 实现过一个异步双缓冲机制的日志系统(前面工作线程写入日志到缓冲区A,后台线程将缓冲区B(3秒缓冲区A会将数据写到B,A写满了也会写到B)中的日志写入数据库(3秒写一次到数据库或者B满了也会写入数据库))，对比过其与 Redis 的写入性能，在批量插入场景下吞吐量(通过测试时间,即测每秒写入的键值对数量)提升了约3倍->这个日志系统是测试的磁盘文件对于两个不同数据库时的一个写入性能->统计一分钟写入到磁盘的键值对数,然后除以60,就得到模拟的写入吞吐量值
