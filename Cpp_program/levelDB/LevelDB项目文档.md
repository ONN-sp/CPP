...
4. 如果位数组存储空间`m`过小,则很快所有的位都会置为1,那么查询任何值都会返回`true`,起不到过滤的作用.同时,如果哈希函数个数`k`越多,布隆过滤器的效率越低.额如果哈希函数太少,则误报率又会增大,因此要按照`k=(m/n)ln2`
5. `CreateFilter()`方法返回过滤器内容:事实上,对每个键只进行一次哈希运算,后续的哈希值是捅第一次的哈希值计算出来的`delta`来得到的,即后续次数的哈希结果是通过前一次的哈希值+`delta`来得到的s
...
3. 我曾用 LevelDB 实现过一个异步双缓冲机制的日志系统(前面工作线程写入日志到缓冲区A,后台线程将缓冲区B(3秒缓冲区A会将数据写到B,A写满了也会写到B)中的日志写入数据库(3秒写一次到数据库或者B满了也会写入数据库))，对比过其与 Redis 的写入性能，在批量插入场景下吞吐量提升了约 3 倍
   ![](markdown图像集/2025-04-01-22-21-56.png)
   日志写入:调用`LevelDB`的批量写入
   日志查询:调用`LevelDB`的迭代器进行查询
   日志管理:持久化、紧凑化等操作
4. LevelDB 的 Compaction 是单线程的，而 RocksDB 通过多线程和可配置策略优化了这一点，这对高并发场景至关重要
