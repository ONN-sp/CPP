...
4. 我曾用 LevelDB 实现过一个异步双缓冲机制的日志系统(前面工作线程写入日志到缓冲区A,后台线程将缓冲区B(3秒缓冲区A会将数据写到B,A写满了也会写到B)中的日志写入磁盘与 Redis AOF持久化(appendfsync=always(没有数据丢失,最安全)和Redis appendfsync=everysec)的写入性能，相比于前者提升了约8倍,相比于后者提升了约3倍->这个日志系统是测试的磁盘文件对于两个不同数据库时的一个写入性能->统计一分钟写入到磁盘的键值对数,然后除以60,就得到模拟的写入吞吐量值
...
13. `LevelDB`不支持事务,它是一个轻量级的嵌入式键值存储引擎,事务在维持其`ACID`特性时需要开销,因此和`LevelDB`的设计初衷相悖
