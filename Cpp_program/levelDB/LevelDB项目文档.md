...
2. 核心设计思想:
   * LevelDB 的核心设计目标是解决传统 B-Tree 类存储引擎的写放大问题。它通过将随机写转换为顺序写（MemTable 内存写入 + WAL 日志持久化），极大提升了写入吞吐量
...
3. 我曾用 LevelDB 实现过一个简单的日志系统，对比过其与 Redis 的写入性能，在批量插入场景下吞吐量提升了约 3 倍
