...
3. 为了更清晰的想清楚迭代公式,可以利用`nullptr`,并将它想做`head`的前一个节点,并且这样不用在外面显示重新设翻转后的节点的`.next`成员为`NULL`
...
2. 递归法:
   ...
   ```
3. <span style="color:red;">对于涉及到链表删除、改变链表节点指向的问题,可以尝试使用虚拟节点(即初始化一个`ListNode* dummyHead=new ListNode(0, head)`).这样可以使原链表的所有节点都按照统一的方式进行操作</span>