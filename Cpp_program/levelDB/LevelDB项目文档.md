...
29. `DoCompactionWork()`大致的执行步骤:
    ...
    如果当前有其它并发读操作要读历史版本，但是这次compaction操作会不会在另一个线程中把历史版本删了，因为来新版本了？
    不会的，正在并发执行的读操作一定持有一个 snapshot，该 snapshot 对应的序列号 ≥ 这次compaction的最小序列号值（因为compaction的最小序列号值就是从快照中的最小的序列号中选的）。其它并发读操作需要的历史版本的序列号一定 ≥ 下界，不会被 compaction 清理掉
...
6. `leveldb`存储引擎和`innodb`存储引擎的不同
...
7. `leveldb`比`innodb`更高性能写入主要是因为:`leveldb`是顺序写，借助了内存中跳表的有序结构；而`innodb`有很多随机写，就算是主键自增id，那么二级索引也会是随机写
8. `leveldb`的高效写入性能的代价：写放大、后台开销大（Compaction：合并、排序）、读性能降低（先从内存读，再读多层的SST文件）
