...
3. `bind、listen、accept、connect`等函数是用户态程序调用的库函数,但是他们会触发系统调用,进入内核态
3. 服务端调用`listen`后`sleep`休眠,客户端的`connect`能成功吗?三次握手能成功吗?
...
4. `accept`是在三次握手之后调用的,它接收的就是三次握手成功的放入`TCP`全连接队列中的`TCP`连接
