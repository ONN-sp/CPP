...
RAPIDJSON_DIAG_POP
    ...
    inline bool Writer<StringBuffer>::ScanWriteUnescapedString(StringStream& is, size_T length){
        ...
        for(;p!=endAligned;p+=16){
            const uint8x16_t s = vldlq_u8(reinterpret_cast<const uint8_t*>(p));
            uint8x16_t x = vceqq_u8(s, s0);// 使用vceqq_u8比较字符是否为特殊字符s0
            x = vceqq_u8(x, s1);// 使用vceqq_u8比较字符是否为特殊字符s1
            x = vceqq_u8(x, s2);// 使用vceqq_u8比较字符是否为特殊字符s2
            x = vceqq_u8(x, s3);// 使用vceqq_u8比较字符是否小于控制字符界限s3(小于控制字符界限就说明它是控制字符)
            x = vrev64q_u8(x);// 对x进行64位反转并提取每8位的结果
            uint64_t low = vgetq_lane_u64(vreinterpretq_u64_u8(x), 0);// 反转后的64位低位,代表检测结果的掩码
            uint64_t highh = vgetq_lane_u64(vreinterpretq_u64_u8(x), 1);// 反转后的64位高位,代表检测结果的掩码
            SizeType len = 0;
