...
7. 为什么innodb只需要有readview一种快照，没有类似version的这种文件视图呢？
   因为innodb采用了undo log版本链管理，版本都通过回滚指针进行链接，可以只通过readview就知道应该读取这条链上的哪一个记录；而对于leveldb，如果没有version就可能导致可以被snapshot读取的数据已经被物理删除了（因为需要version中每个SST的引用计数来管理）（当一个 Snapshot（seq=100）存在时，所有包含了序列号 <= 100 的数据的 SSTable 文件所在的 Version 都会被保留（引用计数增加），表示这个SST被引用+1）
...
17. `LevelDB`的单线程指的是单线程串行写入（从用户角度好像就是可以并发写入）,而其读取仍然是可以多线程并发执行的(leveldb的并发读可以安全，不需要加锁，和mysql的快照读一样，都是利用了mvcc机制),利用了`MVCC`机制,每个读操作基于一个固定的序列号`Sequence number`，通过 版本号（Sequence Number）（等价mysql的事务id，快照等价于readview）和 双向链表管理（等价Mysql的undo log版本链）来实现(读取指定key中的sequence number)来访问特定`version`的数据(小于等于这个快照的sequence number的version对应的数据，即在这之前已经存在的版本),保证一致性(`mysql`中`MVCC`保证一致性是通过事务`id`来和`Readview`中的四个字段进行判断的,来判断出当前事务可以访问的`undo log`版本记录)
18. `LevelDB`的并发写使用了队列锁+添加变量+队列（mysql的并发写是基于的锁，即当前读操作（insert、update等））,所有写线程都会用一把队列锁来写入队列，都写入进去后，只有队首线程没有阻塞，其它都被条件变量cv.wait()阻塞，然后就可以依据队列顺序依次执行写入操作了，即WAL+写入Memtable操作了
