...
2. 本项目的关键性能优化方法:
   ...
   * 字符串处理算法:优化算法处理转义字符
   * 本项目中对于`Value`<=>`GenericValue`对象使用了转移语义(`Move()`).这样做对性能十分友好,这样就可以减少大量没必要的拷贝
...
2. 为什么要使用`JSON`(`XML`)?
   ...
   * `JSON XML`都十分易于阅读和理解,适合开发者直接查看和编辑
   * 无论`JSON XML`,都有大量的库、工具和框架支持它们的解析和生成
3. `JSON`中所有数据都必须嵌套在数组和对象这两种结构中,不能凌驾于之外
4. `JSON`中的对象:
...
4. <mark>为什么`GetString()`函数实现了以`'\0'`结尾的字符串输出,明明先`push`()后`pop()`不应该就算什么都不操作吗?</mark>
...
3. `RapidJSON`相对`JSONCpp`的优势
   * `RapidJSON`解析和生成速度较快，尤其在处理大数据时表现优异
   * 全头文件设计，使用便捷
   * 提供高效的内存管理和多线程支持，适合高性能需求
   * 使用自定义内存分配器，减少内存碎片化，内存使用效率高
   * 支持多线程处理，线程安全
4. `RapidJSON`自定义内存分配器的关键:
   * 内存池存储结构
     - MemoryPoolAllocator 内部使用一个单向链表来存储分配的内存块.每个内存块都有一个头部`ChunkHeader`结构体来记录其容量和已分配的内存大小以及指向下一个内存块的指针
   ...
   * 内存释放机制
     - MemoryPoolAllocator 不会主动释放分配的内存。这是为了减少频繁的内存分配和释放操作可能带来的性能开销和内存碎片化问题. 虽然内存不会被释放，但当用户调用 Free 函数时，MemoryPoolAllocator 会将释放的内存标记为可重用状态，后续的内存分配可以复用这些已释放的内存空间   
