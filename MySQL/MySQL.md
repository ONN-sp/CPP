...
11. MySQL 的 ​Undo Log 清理机制​ 与 LevelDB 的 ​删除标记清理机制​ 在设计思想上高度相似，均采用 ​延迟物理删除​ 策略以优化性能。mysql生成逻辑删除标记（delete_flag），最终的物理清理是在后台的purge线程，它只有等无活跃事务依赖旧版本数据时才清理这个标记了删除的旧版本；leveldb是先写入一个删除标记，然后在compaction中判断是否真正的物理删除记录（根据此时存活的最小快照来判断，如果此时标记了删除的记录的序列化比最小快照小，则可以考虑物理删除）
