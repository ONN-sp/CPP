...
14. 在`insert、update、delete`时都会产生一个`undo log`记录.当`insert`时,产生的`undo log`记录只在回滚时需要,在事务提交后,可被立即删除;而`update、delete`时,产生的`undo log`记录不仅在回滚时需要,在快照读时也需要,不会被立即删除.`InnoDB`的后台线程(`purge thread`)负责清理不再需要的`undo log`记录(update、delete操作产生的undo log记录不能在事务提交后就删除，因为如果删除，那么其他事务就可能就不能回溯到之前有效的undo log记录了。因为通过Read View访问版本链是从最新的开始，如果中间undo log记录被删除，那么就断开了，访问不到后续满足条件的undo log记录了。)
# 日志
