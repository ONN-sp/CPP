...
5. `LevelDB`是一个`C++`编写的高效键值嵌入式存储引擎,目前对亿级的数据有着非常好的读写性能
6. `LevelDB`的优点:
...
3. <mark>`LSM`原理:下图中主要由常驻内存的`C0`树和保留在磁盘的`C1`树两个模块组成.虽然`C1`树保存在磁盘,但是会将`C1`树中一些需要频繁访问的数据保存在数据缓存中,从而保证高频访问数据的快速收敛.当需要插入一条新的数据记录时,首先在`Log`文件末尾顺序添加一条数据插入的记录,然后将这个新的数据添加到驻留在内存的`C0`树中,当`C0`树的数据大小达到某个阈值时,则会自动将数据顺序写到磁盘中的`C1`树,这就是`LSM`树的数据写的全过程.而对于数据读过程,`LSM`树将首先从`C0`树中进行索引查询,如果不存在,则转向`C1`中继续进行查询,直至找到最终的数据记录.对于`LevelDB`来说,`C0`树采用了`Skiplist`数据结构实现,而`C1`树采用了`SSTable`</mark>
...
5. `MNIFEST`主要是用于version恢复用的，因为versionset是易失性的，而`MANIFEST`是存到磁盘的
# CURRENT
...
24. `Level 0`的单个文件中的键是有序的,但在`Level 0`中的所有文件可能会出现键重叠,`Level 0`的`SSTable`文件是直接由内存数据刷盘生成的.而从`Level 1`到`Level 6`,不只单个文件中的键是有序的,每个层级中的所有文件也不会有键重叠,`Level 1`到`Level 6`的所有文件键范围没有重叠(`Level>=1`无重叠指的是它们单独自己的层没重叠,跨层还是可能有重叠的,所以才有`compaction`)
...
32. `DoCompactionWork()`中的`drop`变量表示当前键是否丢弃,两种情况会丢弃:(需要注意:如果有快照,则取最老的快照号,即最小的序列号,然后比它大的序列号的键都不能删,因为旧版本此时可能在被其它线程读)
...
6. 跳表的插入是先执行一遍查询的过程，中途记录新节点是要插入哪一些节点的后面，最后再执行插入。删除是先执行一遍查询的过程，中途记录要删的节点是再哪一些节点的后面，最后再执行删除  
7. 随机产生层高的概率分布公式:
   ![](markdown图像集/2025-03-01-21-37-06.png) 
8. `LevelDB`中的`SkipList`是单向多层有序链表,每个节点中只有后向指针,没有前向指针,因此正向遍历`Next()`明显优于反向遍历`Prev()`
# coding.h/coding.cc
...
9. arena只会产生内部碎片，疑问：但是最后从整个Arena看，不是还是有很多零散空闲小块吗？解答：那些“每个 block 末尾剩下来的几十~几百字节”已经不再是“空闲块”，它们仍然属于 Arena 的那块 4 KB block，Arena 绝不会把它们交还给系统，也不会再被别的对象拿去用，即每个block浪费的内存不是空闲块，它们不是独立的空闲块，它们是随着这个block的分配而分配，随着block释放而释放，它是不会单独被操作系统操作的
# cache.h/cache.cc
...
2. `LRUCache`是包括哈希表(`HandleTable`)的,其关系图如下:
   ...
   通过一个哈希表快速定位到缓存,当发生哈希冲突时使用单向链表解决冲突.除此,哈希表中的所有值都会通过双向链表(`lru_`、`in_use_`)串联.每次访问一个值后,会将值在双向链表中移动位置来表明该值最近被访问,淘汰时直接淘汰一个最久未被访问到的值
3. `LRUCache`由两部分结构组成:
   * `Hash Table`:哈希表
...
18. `data block`内的查找逻辑:二分查找重启点数组+线性查找键值对区间
...
3. `LevelDB`为什么要用`version`?
   `version`可以在文件不断被后台`compaction`增删改的同时,为读取提供一个文件视图.如果没有version的话就不知道此时磁盘上SST在哪一层，key的范围是怎样的，只能全量扫描暴力打开，导致时间变得很长；version里面记录了每个SST的引用次数（每个SST的元数据中），如果没有version的话，就可能导致compaction删除时把本来在被其它线程读取用的SST给删除了，导致不一致性
4. 根据某个Version读时，Compaction会把其中的某个SST文件删除吗，导致version读不到了？
   不会的，因为某个线程根据version读取时就会使对应的SST的引用计数+1，因此compaction时肯定不会删除这个SST，因为其引用计数不为0
...
11. `Version`中的信息（version就是读操作的依据，如果没有version）
   * 每一层文件元数据：有哪些SST文件；每个SST文件的元数据：文件大小、文件key范围、文件编号、每个SST的引用计数等
   * 引用计数(一个version只有引用计数为0时才被删除)
   * 前向指针和后向指针(指向前一个version和后一个version)
# Env.v
...
2. 顺序写:指将数据按照逻辑上的先后顺序，依次连续地写入存储介质的物理位置。这种写入方式通过减少存储设备（如硬盘或固态硬盘）的寻址开销(顺序写避免了频繁的磁盘寻道,顺序写的话就是把接下来要写入的数据直接沿着后面写,磁道移动最多一个位置)，显著提高写入效率和吞吐量(若数据分散在多个不连续的磁道,每次写入需频繁寻道(从一个磁道移动到另一个磁道需要几毫秒-十几毫秒),顺序写磁头只需沿单一方向移动)
3. 核心设计思想:
   ...
   *  Compaction机制：LevelDB 采用分层合并策略：优化读取效率，减少读放大（因为合并SST文件使读取时读的更少了），但也会带来多次磁盘io的写开销，增加了写放大
   * 读优化手段：利用布隆过滤器快速判断 Key 是否在 SSTable 中，避免无效磁盘访问；LRU Cache 缓存热数据，Table Cache 缓存文件句柄，减少 IO 次数
   * 版本控制：通过多版本并发控制(MVCC)实现快照隔离，保证一致性
...
5. `LevelDB`为什么要用`version`(version保存了每个SST的元数据)?
   `version`可以在文件不断被后台`compaction`增删改的同时,为读取提供一个文件视图.如果没有version的话就不知道此时磁盘上SST在哪一层，key的范围是怎样的，只能全量扫描暴力打开，导致时间变得很长；version里面记录了每个SST的引用次数（每个SST的元数据中），如果没有version的话，就可能导致compaction删除时把本来在被其它线程读取用的SST给删除了，导致不一致性
6. LevelDB的读取操作和innodb一样，都要基于某个快照（innodb就是readview），如果没有手动指定`db->GetSnapshot()`,那么就默认使用最新快照;也可以手动指定。LevelDB的快照有两个层面，一个是用户层快照，也就是`db->GetSnapshot()`得到的，它只包含一个序列号，表示我能看到最新的数据就到这了；还有一个是SST层快照，这是由VersionSet管理的，每一个Version就是一个快照，每个Version记录了当前所有SST文件的元信息（文件号、层级、key范围、文件大小等），每次compaction都会生成一个新的version，即version提供的是一个文件视图。LevelDB每次读取都必须同时用到用户层快照和SST层快照，否则无法确定时间点，也无法确定到哪些文件里去找
7. 为什么innodb只需要有readview一种快照，没有类似version的这种文件视图呢？
   因为innodb采用了undo log版本链管理，版本都通过回滚指针进行链接，可以只通过readview就知道应该读取这条链上的哪一个记录；而对于leveldb，如果没有version就可能导致被snapshot读取的数据已经被物理删除了（因为需要version中每个SST的引用计数来管理）（当一个 Snapshot（seq=100）存在时，所有包含了序列号 <= 100 的数据的 SSTable 文件所在的 Version 都会被保留（引用计数增加），表示这个SST被引用+1）
8. 根据某个Version读时，Compaction会把其中的某个SST文件删除吗，导致version读不到了？
   不会的，因为某个线程根据version读取时就会使对应的SST引用计数+1，因此compaction时肯定不会删除这个SST，因为其引用计数不为0
9.  适合写密集、数据冷热分明的场景（如日志存储），但读延迟可能不稳定（需多层查询）
10. `leveldb`存储引擎和`innodb`存储引擎的不同
    * 前者是嵌入式键值存储引擎;后者是关系型数据库存储引擎
    * 前者的数据结构是LSM树;后者是B+树
    * 对于写入:前者是通过内存MemTable缓冲，定期刷盘为SST文件，并通过Compaction合并分层文件;后者按主键顺序存储在B+树叶子节点,支持二级索引，`innodb`是就地更新B+树（可能会经过Buffer pool）
    * 前者的写入吞吐很高，直接是顺序写；后者大多是随机写，存在页分裂开销
    * 前者后台开销比较大，主要是多层SST文件的Compaction；后者相对较低，主要是脏页刷写
    * 前者写放大高（数据会被多次写，SST的Compaction中一个数据可能被合并好几次写到新的SST，即写放大）；后者低
    * 前者不支持事务，无隔离级别（单线程写入）；后者均支持
    * 两者都利用了WAL机制
    * 前者更适用于高吞吐写入、冷热数据分明、嵌入式kv存储、轻量级场景；后者更适用于强事务、复杂查询支持、高并发，关系型表格存储的场景
11. `leveldb`比`innodb`更高性能写入主要是因为:`leveldb`是顺序写，借助了内存中跳表的有序结构；而`innodb`有很多随机写，就算是主键自增id，那么二级索引也会是随机写
12. `leveldb`的高效写入性能的代价：写放大、后台开销大（Compaction：合并、排序）、读性能降低（先从内存读，再读多层的SST文件）
13. `LevelDB`是怎么通过双向链表实现`LRU`结构的?
...
14. `MVCC`的理解:
   MVCC的核⼼思想是每个事务在数据库中看到的数据版本是事务开始时的⼀个快照，⽽不是实际的最新版本。这使得多个事务可以并发执⾏，⽽不会互相⼲扰
15. leveldb的快照需要用户显式创建`GetSnapshot()`，此时会把此刻的全局序列号(快照用的序列号来自内存中的全局计数器(versions_->LastSequence() 返回的是已经提交的最大序列号(或即将写入的下一条))而不是从 key 里“抠”出来的,key 里存的是“这条记录被写入时的序列号”)记录下来并返回一个快照，这一点和innodb不同
16. leveldb的快照就是一个固定时间点的只读视图
17. `LevelDB`的单线程指的是单线程串行写入（从用户角度好像就是可以并发写入）,而其读取仍然是可以多线程并发执行的(leveldb的并发读可以安全，不需要加锁，和mysql的快照读一样，都是利用了mvcc机制),利用了`MVCC`机制,每个读操作基于一个固定的序列号`Sequence number`，通过 版本号（Sequence Number）（等价mysql的事务id，快照等价于readview）和 双向链表管理（version是双向链表管理的）（等价Mysql的undo log版本链）来实现(读取指定key中的sequence number)来访问特定`version`的数据(小于等于这个快照的sequence number的version对应的数据，即在这之前已经存在的版本),保证一致性(`mysql`中`MVCC`保证一致性是通过事务`id`来和`Readview`中的四个字段进行判断的,来判断出当前事务可以访问的`undo log`版本记录)
18. `LevelDB`的并发写使用了队列锁+条件变量+队列（mysql的并发写是基于的锁，即当前读操作（insert、update等））,所有写线程都会用一把队列锁来写入队列，都写入进去后，只有队首线程没有阻塞，其它都被条件变量cv.wait()阻塞，然后就可以依据队列顺序依次执行写入操作了，即后续的WAL+写入Memtable操作了
19. `LeveDB`是单机存储的,单机!=单线程,单机指的是不能分布式
20. `LevelDB`不支持事务,它是一个轻量级的嵌入式键值存储引擎,事务在维持其`ACID`特性时需要开销,因此和`LevelDB`的设计初衷相悖
21. 在`LevelDB`中,如果多个线程同时调用写入接口`Put()/Delete()`,`LevelDB`内部会将这些请求串行化(按到达顺序排队),会写入一个批量写入队列(所有线程写入队列这个过程是需要加锁的，即入队操作是串行的),逐个执行(最终效果等同于所有写入由一个线程实际执行,其他线程需等待)，这里的过程是：所有写入线程是并发将写入命令（一个Writer对象）插入队列，然后，通过条件变量调用wait将该线程挂起，等待其它线程发送唤醒信号，并且等到队列前面的写入命令操作全部执行完毕。如果此线程被唤醒了，但是队列前面仍有其它Writer操作，那么该线程会再次wait阻塞，从而保证Writer操作按照队列生成次序执行。为什么要这样？
    * 直接单线程顺序写就能保证原子性和一致性
    * 为了保证顺序写，就需要加锁，加锁不仅有锁资源占用，而且上下文切换会带来很大消耗
    * 天然保证sequencenumber是单调递增的（因为如果不是递增就和最新版本记录的序列号更大相违背了），如果是并发写就要是原子自增
22. 磁盘文件`SSTable`的`Level 1-6`层的有序是在`Compaction`过程中利用归并排序实现的
23. 我自己对leveldb项目的修改：为了防止在一个长时间运行过程中，磁盘容量暴涨，我对每个键值对增加了一个TTL字段，添加到了key中sequence number后面，即后面读取的时候不仅会根据sequence number来读取最新的可见版本，而且还要满足这个版本未过期。过期扫描是开一个线程定时30分钟去扫描。此修改有一个问题：如果一个键值对的最新版本都过期了，那么就是读不到这个版本了，此时会返回已过期
24. leveldb的锁主要是：队列锁、分片锁，队列锁是用于保证按照队列顺序执行写入操作，从而看着就像是单线程写入，其实是串行化操作；分片锁：用 此时 key 的哈希值取模 16，映射到 16 个独立的互斥锁之一，leveldb只有16把分片锁，所有 key 按照哈希值映射到这 16 个锁上
25. leveldb真正的删除是在SSTable的compaction中，在内存中是写的一个删除标记的操作，Compaction的删除：
    * 找当前存活的最老快照，得到其序列号
    * 对SST进行归并排序，并返回迭代器
    * 选取键之后，如果是重复写入一个键，那么可以删除这个键比最老快照的序列号还小的键；如果是一个写入的删除标记，那么当高层无此键的时候，也可以彻底删除，但也要这个键的序列号比最老快照的小
26. leveldb是怎么删除kv的=>写入一个删除标记的方式，不是原地删除，为什么这样
    * 写入性能高：删除操作和写入操作统一，只是写入一个特殊的标记，而不需要查找或覆盖元数据，这是一个顺序写过程，性能极高
    * 简化系统设计，不需要定位并删除磁盘上的数据，避免了复杂的IO
    * 删除标记会随着Compaction过程被清理，只有比此时存活的最小的快照序列号小的才能从磁盘中清理
    * 删除标记的存在使得快照读取时能准确判断某个key在某个时间点是否存在，即在这个删除标记之前的快照可以读到历史数据；而在这之后的快照就读不到这个kv了
27. 写入删除标记的方法只会写一次不会读，而原地删除会读一次和写一次（先读找到位置；然后在该block上删除key；再把这个block重新压缩；最后再把这个block写回去）
28. `LSM`树是怎么保证顺序写的?
    * `Memtable`：LSM树首先将数据写入内存中的Memtable。Memtable是一个有序的数据结构（跳表），它按照键的顺序存储数据
    * `SSTable`：当Memtable达到一定大小时，它会被顺序刷写到磁盘，形成一个SSTable文件。SSTable文件是按键顺序排列的，且数据是追加写入的，写入时磁盘不会随机寻道,而是沿着单一方向寻道
29. `LevelDB`中的`MemTable`利用`Arena`内存池进行内存管理?
