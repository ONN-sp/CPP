...
6. 基于我的电脑`i5-9Gen`,4核8线程,压测`QPS`的结果基本达到3w左右
# 好用的工具
1. `top`:观察`cpu`、内存等占用率
...
# 面试项目介绍
1. 该项目是一个基于 Reactor 模式的 C++ 高性能网络库，核心设计是 One Loop per Thread。主线程监听新连接，子线程处理 I/O。例如，每个 TcpConnection 绑定到一个 EventLoop，利用非阻塞 I/O 和水平触发模式，配合应用层缓冲减少系统调用。在压测中发现，可以支持万级的并发请求（这是单机的）
...
3. 该项目主要技术点：
   ...
   * 主线程负责监听并接收连接（Acceptor），然后用轮询的方式将任务分给子线程，实现负载均衡，子线程处理已连接套接字的 I/O，适合高并发场景
   * 整个项目都是基于RAII机制,即将资源(锁、条件变量、文件描述符等等)的获取和释放绑定到对象的生命周期
   * 对于异步日志库,采用双缓冲机制
   * 采用了很多内置的系统函数,如syscall memcpy()等,这样能提高效率
   * 对于解决文件描述符耗尽的情况(也可说成是处理busy loop),提前准备空文件描述符,在面对服务器文件描述符耗尽时采用关闭/再接收/再关闭的方法,处理了此时的busy loop
4. 该项目是IO密集型WebServer，基于事件驱动+非阻塞IO，主要性能指标是并发连接数、QPS等，不关心cpu利用率，在压测并发长连接时此项目的工作线程的cpu利用率都只有60%左右；计算密集型连接数通常不高，目的是让CPU尽量满载。短连接时，各线程的CPU负载比较均衡，差不多为25%左右（主线程50%，其余25%左右）；长连接时，主线程负载基本为0，线程池的线程负载60%左右，因为没有新的连接需要处理
5. 长连接能处理的请求数是20000多，而短连接却1w不到，因为长连接没有了连接建立和断开的开销
6. 此项目QPS在3w左右，子线程池的线程数为4
7. 压测时看到的每个线程只占0.3%左右mem的内存，因为：线程真正独占的物理页，通常只有「几十 KB 的线程栈 + 几 KB 的 TLS + 若干 KB 的 malloc arena」；其余大块（socket 缓冲、共享库代码）都是进程级共享，因此 top 里看到的 MEM 才会是零点几
8. 我主要就是进行了小的修改：
   * 通过swap()函数来将共享资源拷贝到局部变量，从而减小锁的范围；
   * 使用移动语义的move函数；
   * 对此项目的工作线程引入了优先级字段，通过大顶堆priority_queue实现的，这样可以在主线程建立连接后就优先调度到优先级高的子任务进行执行；
9. 设计代码时尽量要模块解耦:将代码分解成一个个独立的模块,每个模块负责不同的功能.这样可以降低代码耦合度,使得代码更易于维护和修改,同时也可以提高代码的可复用性和可扩展性
10. 在此过程中我所遇到的问题:
    ...
    * 压测效果不好:实验了很多次,比如:自己写的代码部分导致性能不好,然后对着源码敲了很多次发现没用;文件描述符设置等问题(`Keep-Alive`、文件描述符默认打开1024,将它增多)、临时对象创建太多).这样尝试都没用,最后发现主要问题是`Cmake`中写的编译优化问题,使用了调试模式(之前输出调试信息忘记关了),需要将其改为`Release`(Debug模式下编译器不会进行优化;Release模式下编译器会进行优化),因为它生成的代码经过优化并禁用调试信息
    * 出现死锁:使用`Valgrind Helgrind`工具
    * 刷新日志到磁盘,出现乱码:传入的写入长度出问题,导致指针移动到未定义位置,从而写入乱码
11. webserver没有内存池，buffer是针对每个tcp连接设计的应用层缓冲区。output buffer：当应用程序调用 send() 发送数据时，操作系统可能因内核发送缓冲区不足或 ​TCP 窗口限制而无法一次性全部发送，此时就需要output buffer保存剩下的数据；Tcp是一个无边界的字节流协议,接收方可能出现粘包和分包的问题,因此需要input buffer（暂存不完整的数据，等待后续数据到达，拼成完整消息后再通知业务逻辑处理；也可以处理将未按序到达的数据进行重组）
12. 传统的多生产者多消费者模式维护的任务队列不管是入队还是出队都是需要加锁的（入队和出队是同一把锁，因为是同一个共享资源），这个任务队列是所有线程所共享的共享资源（不加锁会怎样？比如：生产者A 看到 size=5，准备将新元素放入 index=5 的位置；就在此时，生产者B 也看到了 size=5，也准备将元素放入 index=5 的位置；结果导致其中一个元素会被另一个覆盖，造成数据丢失，同时size更新也出错。生产者正在向队列插入一个复杂对象，写入操作不是原子的，它可能分多步完成；消费者线程可能在生产者只写了一半的时候读取了这个对象，读到的数据是破损的）
13. 我所设计的webserver是IO密集型的，主线程负责监听连接并处理，然后将这个连接（比如每个连接的HTTP的get请求等）轮询分发（负载均衡）给不同的子线程（子线程是一个线程池，线程池中的线程数=4）（这个子线程负责这个连接的后续所有IO事件）。每个线程是有一个自己的任务队列std::vector<std::function<void()>>（在我自己实现里面，我给每个vector传入的是一个pair类型，第一个元素是优先级，用int表示，第二个元素是任务std::function<void()>），而不是和传统的生产者消费者模式中共享一个任务队列一样，这里是每个线程维护着自己独立的任务队列，这是webserver高性能的原因之一，它极大减少了线程间的锁竞争（因为如果是共享一个任务队列，那么不管是任务入队还是出队都需要锁竞争（就是多个线程都去抢这个锁资源））