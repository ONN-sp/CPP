...
void HTTP_server::execute_cgi(int sock, const char* path, const char* method, const char* query_string){
    ...
    if(pid==0){//pid==0是用来确定当前是子进程.  子进程:执行CGI程序
        ...
        execl(path, path, nullptr);//执行CGI脚本,它的输出会被写入到cgi_output管道中(因为标准输出被重定向了) 
        exit(0);   
    }
    else{//父进程
...
int main(){
    ...
    while(1){
        ...
        for(int i=0;i<num;i++){
            ...
            if(fd == server_sock){
                if((cfd = accept(server_sock, (struct sockaddr*)&client_name, (socklen_t*)&addrlen))<0){
                ...
                pool.enqueue(std::bind(&HTTP_server::accept_request, &http, cfd), cfd);
            }
        }
    }      
    close(server_sock);
