...
3.  类的前向声明:这种情况通常发生在类之间存在相互引用的情况下,`class className;`.<mark>前向声明简化了头文件直接的依赖关系.对于现代的`GNU`的预处理过程,它在第二次会对同一个头文件不会去读取它,而是直接跳过,这样就加快了编译速度</mark>,一般是为了在`A`头文件中不想引入`B`的头文件,但要使用`B`类,并且在`A`中只会使用这个类型类`B`(即不会使用`B`的对象),而不需要它的定义,所以此时可以使用前向声明
    ```C++
